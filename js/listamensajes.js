function crearLista() {
		...
		$.ajax({
		url: 'http://appcoplp.micolegioapp.com/php/listamensajes.php',
		dataType: 'jsonp',
		jsonp: 'jsoncallback',
		timeout: 5000,
	    ...
	    $.each(data, function(index, item) {
			...
			$(listo).append(			
       		...
       		'<li class="ui-li-has-alt ui-li-has-thumb ui-first-child ui-last-child" style="height: 60px"><a href="#" class="ui-btn" onclick="changePage(id_' + index + '), contador(\''+registro+'\');" style="height: 60px; padding:0; background-color:#ffffff " >'+
       ...
       '<a href="#" data-rel="popup" data-position-to="window" data-transition="pop" aria-haspopup="true" aria-owns="purchase" aria-expanded="false" class="ui-btn ui-btn-icon-notext ui-icon-delete ui-btn-a" onClick="borrarMensaje(\''+item.id_aux+'\' )" style="height: 62px; background-color:#ffffff">'+
		'</a>'+
    	'</li>'
		...
		/*Pagina dinamica*/
			...
			'<div data-role="page"  id="id_' + index + '" data-url="id_' + index + '" data-theme="a" >' +
                    ...
                    '<p style="margin:0; padding:0; color:#005099; font-size: 0.9em ">Nº de Registro: '+item.registro+'<br>'+
                    'Fecha: '+item.fecha+'</p>'+
			...
			$('#piepagina').append('<div data-role="footer" data-theme="a" data-position="fixed" role="contentinfo" class="ui-footer ui-bar-b ui-footer-fixed slideup ui-fixed-hidden">'+
...
function modificaEstado(registro){
	...
	$.ajax({
		url: 'http://appcoplp.micolegioapp.com/php/modificaestado.php',
		dataType: 'jsonp',
		jsonp: 'jsoncallback',
		timeout: 5000,
...
function contador(registro){
	...
	$.ajax({
		url: 'http://appcoplp.micolegioapp.com/php/contador.php',
		dataType: 'jsonp',
		jsonp: 'jsoncallback',
		timeout: 5000,
...
function descargarArchivo(nombre){
                ...
                var server = "http://micolegioapp.com/coplp/web/files/";
                var filename = nombre;
                ...
                function onFileSystemSuccess(fileSystem) {
                    ...
                    fileSystem.root.getDirectory('COPLP',
                        { create:true },
                        transferFile,
                        onErrorCallback
                    );
